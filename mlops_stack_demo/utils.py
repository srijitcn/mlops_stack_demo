...
def _get_ml_config_value(env, key):
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"mlops_stack_demo have been deployed to {env} (see mlops_stack_demo"
...
def get_model_name(env, test_mode=False):
    ...
    if env == "dev" or test_mode:
        ...
        return f"mlops_stack_demo-model{resource_name_suffix}"
    else:
        # Read ml model name from mlops_stack_demo/terraform
        return _get_ml_config_value(env, "mlops_stack_demo_model_name")
